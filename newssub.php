<?php
            ...
            if($rows==0) {
                print('<div class="dlike">Вы ни на кого не подписаны</div><style>.nosub {display:none;}</style>');
                $result->data_seek(0);
            ...
            $query = $query2.' order by id DESC LIMIT 5';
            $result = $conn->query($query);
            $rows = $result->num_rows;
            $result->data_seek(0);
            $_SESSION['moresub'] = $result->fetch_array()['id'].' '.'5';
            if($rows==0) {
                print('<div class="dlike nosub">У ваших друзей нет новых записей</div>');
            }
            for($j = 0;$j < $rows;$j++) {
                    $result->data_seek($j); 
                    ...
                    $query2 = "SELECT * from news WHERE realid='".$realid."' AND reblog !='' AND locked!='true'"; 
...
<script type="text/javascript">
    ...
    document.title = 'ListSend | Читаемые';
