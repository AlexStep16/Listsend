...
<?php
    ...
    if($date == '10') {
        return 'октября';
            ...
            # Узнать какие записи мне не нравятся
            ...
            for($c = 0; $c<$rowsanswer; $c++) {
                    $answer->data_seek($c); 
                    # Не нравится определенный логин
            ...
            $query = "SELECT * from news WHERE reblog=''".$disstop." AND login!='".$_SESSION['login']."' ".$query2." AND locked!='true' order by id DESC";
            $result = $conn->query($query);
            $rows = $result->num_rows;
            $result->data_seek(0);
            $_SESSION['more'] = $result->fetch_array()['id'].' '.'5';
            if($rows==0) {
                print('<div class="dlike nosub">Для вас не найдено рекомендаций :(</div>');
            }
            for($j = 0;$j < $rows;$j++) {
                    $hr2="<hr>";
                    $hr="<hr>";
...
<script type="text/javascript">
    ...
    document.title = 'ListSend | Рекомендации';
</script>