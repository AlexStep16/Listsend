...
if(isset($_POST['name'])) {
            ...
            $_POST['sod'] = addslashes($_POST['sod']);
            $_POST['q'] = strip_tags($_POST['q']);
            $_POST['q'] = addslashes($_POST['q']);
            $_POST['q'] = str_replace('&nbsp;', ' ', $_POST['q']);
            $_POST['q'] = str_replace('#', '', $_POST['q']);
            $query = "INSERT into news(login,email,name,sod,tags,factdate,locked,comments) VALUES('".$_SESSION['login']."','".$emailfor."','".$_POST['name']."','".$_POST['sod']."','".$_POST['q']."','".$factdate."','".$_POST['locked']."','".$_POST['comments']."')"; 
            $result = $conn->query($query);
            $query = "SELECT * from news WHERE login='".$_SESSION['login']."' AND email='".$emailfor."' order by id DESC LIMIT 1";
            $result = $conn->query($query);
            $result->data_seek(0);
            ...
            $result->data_seek(0);
            $array = explode('***',$_GET['img']);
            $ro = count($array);
            for($j=0;$j<$ro;$j++) {
                if(is_file('tmp/'.$array[$j])) {
                    moveFile( 'tmp/'.$array[$j], 'noteimage/');
                    $result->data_seek(0);
                    rename('noteimage/'.$array[$j],'noteimage/'.$j.md5($result->fetch_array()['id']).'.jpg');
                }
            }
            ...
            for($j = 0;$j < $rows;$j++) {
                    ...
                    if($del == 0) {
                        $divlike = '<span class="like like'.$id.'" onclick="like(\''.$id.'\'); changelike(\''.$id.'\');"><span class="likef likef'.$id.'"><span class="likefcount'.$id.'">'.$likes.'</span> Лайков</span><span class=" imglike'.$id.'"><img src="https://png.icons8.com/windows/21/000000/hearts.png"></span>&nbsp;</span>';
                    }
                    # Если мой лайк стоит
                    else {
                        $divlike = '<span class="like like'.$id.'" onclick="like(\''.$id.'\'); changelike2(\''.$id.'\');"><span class="likef likef'.$id.'"><span class="likefcount'.$id.'">'.$likes.'</span> Лайков</span><span class=" imglike'.$id.'"><img src="https://png.icons8.com/color/20/000000/filled-like.png"></span>&nbsp;</span>';
                    }
                    ...
                    print('
                            <div class="box-line" id="note'.$id.'" style="animation:show 1s ease;">
                                    ...
                                    </div><div style="position:relative;"><div class="tolike tolike'.$id.'"><img src="icons8-love-100.png"></div>'.$hr.$h1_name.$divsod.$div.$divtags.$hr2.'</div><div class="likes"><div class="number"><span class="span"></div><div class="allinlikes">'.$divlike.'<span onclick="docom(\''.$id.'\')" class="commenty"><span class="comf" id="komnum"><span class="countcomf'.$id.'">'.$commentrows.'</span> Комментариев</span><img src="https://png.icons8.com/metro/16/333333/topic.png" style="top:3px;">&nbsp;&nbsp;</span><span onclick="repostconfirm(\''.$id.'\')" class="repost"><span class="repf"><span class="countrepf">0</span> Репостов</span><img src="https://png.icons8.com/material/21/333333/refresh.png"></span></span></div></div><br>');
